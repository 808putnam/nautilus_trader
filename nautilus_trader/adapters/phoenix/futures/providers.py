...
from nautilus_trader.adapters.phoenix.common.enums import PhoenixSymbolFilterType
...
from nautilus_trader.adapters.phoenix.futures.enums import PhoenixFuturesContractType
from nautilus_trader.adapters.phoenix.futures.http.account import PhoenixFuturesAccountHttpAPI
from nautilus_trader.adapters.phoenix.futures.http.market import PhoenixFuturesMarketHttpAPI
from nautilus_trader.adapters.phoenix.futures.http.wallet import PhoenixFuturesWalletHttpAPI
from nautilus_trader.adapters.phoenix.futures.schemas.account import PhoenixFuturesFeeRates
from nautilus_trader.adapters.phoenix.futures.schemas.account import PhoenixFuturesPositionRisk
from nautilus_trader.adapters.phoenix.futures.schemas.market import PhoenixFuturesSymbolInfo
...
class PhoenixFuturesInstrumentProvider(InstrumentProvider):
    """
    Provides a means of loading instruments from the `Phoenix Futures` exchange.
    ...
    def __init__(
        ...
        self._http_account = PhoenixFuturesAccountHttpAPI(
            self._client,
            clock=self._clock,
            account_type=account_type,
        )
        self._http_wallet = PhoenixFuturesWalletHttpAPI(
            self._client,
            clock=self._clock,
            account_type=account_type,
        )
        self._http_market = PhoenixFuturesMarketHttpAPI(self._client, account_type=account_type)
        ...
        # real-time account update messages provided by Phoenix.
        ...
        self._fee_rates = {
            0: PhoenixFuturesFeeRates(feeTier=0, maker="0.000200", taker="0.000400"),
            1: PhoenixFuturesFeeRates(feeTier=1, maker="0.000160", taker="0.000400"),
            2: PhoenixFuturesFeeRates(feeTier=2, maker="0.000140", taker="0.000350"),
            3: PhoenixFuturesFeeRates(feeTier=3, maker="0.000120", taker="0.000320"),
            4: PhoenixFuturesFeeRates(feeTier=4, maker="0.000100", taker="0.000300"),
            5: PhoenixFuturesFeeRates(feeTier=5, maker="0.000080", taker="0.000270"),
            6: PhoenixFuturesFeeRates(feeTier=6, maker="0.000060", taker="0.000250"),
            7: PhoenixFuturesFeeRates(feeTier=7, maker="0.000040", taker="0.000220"),
            8: PhoenixFuturesFeeRates(feeTier=8, maker="0.000020", taker="0.000200"),
            9: PhoenixFuturesFeeRates(feeTier=9, maker="0.000000", taker="0.000170"),
        }
    ...
    async def load_all_async(self, filters: dict | None = None) -> None:
        ...
        for symbol_info in exchange_info.symbols:
            fee = PhoenixFuturesCommissionRate(
    ...
    async def load_ids_async(
        ...
        for instrument_id in instrument_ids:
            PyCondition.equal(instrument_id.venue, PHOENIX_VENUE, "instrument_id.venue", "PHOENIX")
        ...
        symbols = [
            str(PhoenixSymbol(instrument_id.symbol.value)) for instrument_id in instrument_ids
        ]

        # Get exchange info for all assets
        exchange_info = await self._http_market.query_futures_exchange_info()
        symbol_info_dict: dict[str, PhoenixFuturesSymbolInfo] = {
    ...
    async def load_async(self, instrument_id: InstrumentId, filters: dict | None = None) -> None:
        ...
        symbol = str(PhoenixSymbol(instrument_id.symbol.value))
    ...
    def _parse_instrument(
        self,
        symbol_info: PhoenixFuturesSymbolInfo,
        ts_event: int,
        position_risk: PhoenixFuturesPositionRisk | None = None,
        fee: PhoenixFuturesCommissionRate | None = None,
    ) -> None:
        contract_type_str = symbol_info.contractType

        if (
            contract_type_str == ""
            or symbol_info.status == PhoenixFuturesContractStatus.PENDING_TRADING
        ):
        ...
        try:
            ...
            parsed_symbol = PhoenixSymbol(raw_symbol.value).parse_as_nautilus(
                self._account_type,
            )
            nautilus_symbol = Symbol(parsed_symbol)
            instrument_id = InstrumentId(symbol=nautilus_symbol, venue=PHOENIX_VENUE)

            # Parse instrument filters
            filters: dict[PhoenixSymbolFilterType, PhoenixSymbolFilter] = {
                f.filterType: f for f in symbol_info.filters
            }
            price_filter: PhoenixSymbolFilter = filters.get(PhoenixSymbolFilterType.PRICE_FILTER)
            lot_size_filter: PhoenixSymbolFilter = filters.get(PhoenixSymbolFilterType.LOT_SIZE)
            min_notional_filter: PhoenixSymbolFilter = filters.get(
                PhoenixSymbolFilterType.MIN_NOTIONAL,
            )
            ...
            min_notional = None
            if filters.get(PhoenixSymbolFilterType.MIN_NOTIONAL):
            ...
            contract_type = PhoenixFuturesContractType(contract_type_str)
            if contract_type == PhoenixFuturesContractType.PERPETUAL:
                instrument = CryptoPerpetual(
                    ...
                    is_inverse=False,  # No inverse instruments trade on Phoenix
                ...
                PhoenixFuturesContractType.CURRENT_MONTH,
                PhoenixFuturesContractType.CURRENT_QUARTER,
                PhoenixFuturesContractType.CURRENT_QUARTER_DELIVERING,
                PhoenixFuturesContractType.NEXT_MONTH,
                PhoenixFuturesContractType.NEXT_QUARTER,
            ):
                ...
                raise RuntimeError(  # pragma: no cover (design-time error)
                    f"invalid `PhoenixFuturesContractType`, was {contract_type}",  # pragma: no cover
                )
