...
from nautilus_trader.adapters.phoenix.futures.enums import PhoenixFuturesEventType
from nautilus_trader.adapters.phoenix.futures.enums import PhoenixFuturesPositionSide
from nautilus_trader.adapters.phoenix.futures.enums import PhoenixFuturesPositionUpdateReason
from nautilus_trader.adapters.phoenix.futures.enums import PhoenixFuturesWorkingType
...
class PhoenixFuturesUserMsgData(msgspec.Struct, frozen=True):
    """
    Inner struct for execution WebSocket messages from `Phoenix`.
    """

    e: PhoenixFuturesEventType


class PhoenixFuturesUserMsgWrapper(msgspec.Struct, frozen=True):
    """
    Provides a wrapper for execution WebSocket messages from `Phoenix`.
    """

    data: PhoenixFuturesUserMsgData | None = None
    stream: str | None = None


class MarginCallPosition(msgspec.Struct, frozen=True):
    """
    Inner struct position for `Phoenix Futures` Margin Call events.
    """

    s: str  # Symbol
    ps: PhoenixFuturesPositionSide  # Position Side
...
class PhoenixFuturesMarginCallMsg(msgspec.Struct, frozen=True):
    """
    WebSocket message for `Phoenix Futures` Margin Call events.
    """
...
class PhoenixFuturesBalance(msgspec.Struct, frozen=True):
    """
    Inner struct balance for `Phoenix Futures` Balance and Position update event.
    """
...
class PhoenixFuturesPosition(msgspec.Struct, frozen=True):
    """
    Inner struct position for `Phoenix Futures` Balance and Position update event.
    """
    ...
    ps: PhoenixFuturesPositionSide


class PhoenixFuturesAccountUpdateData(msgspec.Struct, frozen=True):
    """
    WebSocket message for `Phoenix Futures` Balance and Position Update events.
    """

    m: PhoenixFuturesPositionUpdateReason
    B: list[PhoenixFuturesBalance]
    P: list[PhoenixFuturesPosition]

    def parse_to_account_balances(self) -> list[AccountBalance]:
        return [balance.parse_to_account_balance() for balance in self.B]


class PhoenixFuturesAccountUpdateMsg(msgspec.Struct, frozen=True):
    """
    WebSocket message for `Phoenix Futures` Balance and Position Update events.
    """
    ...
    a: PhoenixFuturesAccountUpdateData

    def handle_account_update(self, exec_client: PhoenixCommonExecutionClient):
        """
        Handle PhoenixFuturesAccountUpdateMsg as payload of ACCOUNT_UPDATE.
        """
...
class PhoenixFuturesAccountUpdateWrapper(msgspec.Struct, frozen=True):
    """
    WebSocket message wrapper for `Phoenix Futures` Balance and Position Update events.
    """

    stream: str
    data: PhoenixFuturesAccountUpdateMsg


class PhoenixFuturesOrderData(msgspec.Struct, kw_only=True, frozen=True):
    """
    WebSocket message 'inner struct' for `Phoenix Futures` Order Update events.
    ...
    def parse_to_order_status_report(
        ...
        return OrderStatusReport(
            ...
            trigger_type=enum_parser.parse_phoenix_trigger_type(self.wt.value),
    ...
    def handle_order_trade_update(  # noqa: C901 (too complex)
        ...
        """
        Handle PhoenixFuturesOrderData as payload of ORDER_TRADE_UPDATE event.
        """
        ...
        elif self.x == PhoenixExecutionType.CANCELED or (
            exec_client.treat_expired_as_canceled and self.x == PhoenixExecutionType.EXPIRED
        ):
        ...
        elif self.x == PhoenixExecutionType.AMENDMENT:
            instrument = exec_client._instrument_provider.find(instrument_id=instrument_id)
            if instrument is None:
...
class PhoenixFuturesOrderUpdateMsg(msgspec.Struct, frozen=True):
    """
    WebSocket message for `Phoenix Futures` Order Update events.
    """

    e: str  # Event Type
    E: int  # Event Time
    T: int  # Transaction Time
    o: PhoenixFuturesOrderData


class PhoenixFuturesOrderUpdateWrapper(msgspec.Struct, frozen=True):
    """
    WebSocket message wrapper for `Phoenix Futures` Order Update events.
    """

    stream: str
    data: PhoenixFuturesOrderUpdateMsg
