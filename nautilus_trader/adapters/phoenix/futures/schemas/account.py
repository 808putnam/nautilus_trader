...
class PhoenixFuturesBalanceInfo(msgspec.Struct, frozen=True):
    """
    HTTP response 'inner struct' from `Phoenix Futures` GET /fapi/v2/account (HMAC
    SHA256).
    """
...
class PhoenixFuturesAccountInfo(msgspec.Struct, kw_only=True, frozen=True):
    """
    HTTP response from `Phoenix Futures` GET /fapi/v2/account (HMAC SHA256).
    """
    ...
    assets: list[PhoenixFuturesBalanceInfo]
...
class PhoenixFuturesPositionRisk(msgspec.Struct, kw_only=True, frozen=True):
    """
    HTTP response from ` Phoenix Futures` GET /fapi/v2/positionRisk (HMAC SHA256).
    """
    ...
    positionSide: PhoenixFuturesPositionSide
    updateTime: int
    ...
    def parse_to_position_status_report(
        ...
        enum_parser: PhoenixFuturesEnumParser,
        report_id: UUID4,
        ts_init: int,
...
class PhoenixFuturesDualSidePosition(msgspec.Struct, frozen=True):
    """
    HTTP response from `Phoenix Futures` GET /fapi/v1/positionSide/dual (HMAC SHA256).
    """

    dualSidePosition: bool


class PhoenixFuturesFeeRates(msgspec.Struct, frozen=True):
    """
    Represents a `PhoenixFutures` fee tier.
