...
from nautilus_trader.adapters.phoenix.common.constants import PHOENIX_VENUE
...
from nautilus_trader.adapters.phoenix.common.enums import PhoenixErrorCode
from nautilus_trader.adapters.phoenix.common.enums import PhoenixKlineInterval
from nautilus_trader.adapters.phoenix.common.schemas.market import PhoenixAggregatedTradeMsg
from nautilus_trader.adapters.phoenix.common.schemas.market import PhoenixCandlestickMsg
from nautilus_trader.adapters.phoenix.common.schemas.market import PhoenixDataMsgWrapper
from nautilus_trader.adapters.phoenix.common.schemas.market import PhoenixOrderBookMsg
from nautilus_trader.adapters.phoenix.common.schemas.market import PhoenixQuoteMsg
from nautilus_trader.adapters.phoenix.common.schemas.market import PhoenixTickerMsg
...
from nautilus_trader.adapters.phoenix.common.types import PhoenixTicker
...
from nautilus_trader.adapters.phoenix.http.error import PhoenixError
from nautilus_trader.adapters.phoenix.http.market import PhoenixMarketHttpAPI
from nautilus_trader.adapters.phoenix.websocket.client import PhoenixWebSocketClient
...
class PhoenixCommonDataClient(LiveMarketDataClient):
    """
    Provides a data client of common methods for the `Phoenix` exchange.
    ...
    market : PhoenixMarketHttpAPI
        The phoenix Market HTTP API.
    enum_parser : PhoenixEnumParser
        The parser for Phoenix enums.
    msgbus : MessageBus
    ...
    def __init__(
        ...
        super().__init__(
            loop=loop,
            client_id=ClientId(PHOENIX_VENUE.value),
            venue=PHOENIX_VENUE,
            msgbus=msgbus,
            cache=cache,
            clock=clock,
        ...
        # Configuration
        self._phoenix_account_type = account_type
        self._use_agg_trade_ticks = config.use_agg_trade_ticks
        self._log.info(f"Account type: {self._phoenix_account_type.value}.", LogColor.BLUE)
        ...
        # WebSocket API
        self._ws_client = PhoenixWebSocketClient(
            clock=clock,
        ...
        self._decoder_data_msg_wrapper = msgspec.json.Decoder(PhoenixDataMsgWrapper)
        self._decoder_order_book_msg = msgspec.json.Decoder(PhoenixOrderBookMsg)
        self._decoder_quote_msg = msgspec.json.Decoder(PhoenixQuoteMsg)
        self._decoder_ticker_msg = msgspec.json.Decoder(PhoenixTickerMsg)
        self._decoder_candlestick_msg = msgspec.json.Decoder(PhoenixCandlestickMsg)
        self._decoder_agg_trade_msg = msgspec.json.Decoder(PhoenixAggregatedTradeMsg)

        # Retry logic (hard coded for now)
        self._max_retries: int = 3
        self._retry_delay: float = 1.0
        self._retry_errors: set[PhoenixErrorCode] = {
            PhoenixErrorCode.DISCONNECTED,
            PhoenixErrorCode.TOO_MANY_REQUESTS,  # Short retry delays may result in bans
            PhoenixErrorCode.TIMEOUT,
            PhoenixErrorCode.INVALID_TIMESTAMP,
            PhoenixErrorCode.ME_RECVWINDOW_REJECT,
        }
        ...
        while True:
            ...
            while True:
                try:
                    ...
                    error_code = PhoenixErrorCode(e.message["code"])
                    retries += 1
    ...
    def _should_retry(self, error_code: PhoenixErrorCode, retries: int) -> bool:
        if (
    ...
    async def _subscribe(self, data_type: DataType) -> None:
        ...
        if data_type.type == PhoenixTicker:
            await self._ws_client.subscribe_ticker(instrument_id.symbol.value)
        elif data_type.type == PhoenixFuturesMarkPriceUpdate:
            if not self._phoenix_account_type.is_futures:
                self._log.error(
                    f"Cannot subscribe to `PhoenixFuturesMarkPriceUpdate` "
                    f"for {self._phoenix_account_type.value} account types.",
                )
                return
    ...
    async def _unsubscribe(self, data_type: DataType) -> None:
        ...
        if instrument_id is None:
            self._log.error(
                "Cannot subscribe to `PhoenixFuturesMarkPriceUpdate` no instrument ID in `data_type` metadata.",
            )
            return
        ...
        if data_type.type == PhoenixTicker:
            ...
            if not self._phoenix_account_type.is_futures:
                self._log.error(
                    "Cannot unsubscribe from `PhoenixFuturesMarkPriceUpdate` "
                    f"for {self._phoenix_account_type.value} account types.",
                )
                return
        else:
    ...
    async def _subscribe_order_book(  # (too complex)
        ...
        if book_type == BookType.L3_MBO:
            self._log.error(
                ...
                "L3_MBO data is not published by Phoenix. "
                "Valid book types are L1_MBP, L2_MBP.",
            )
            return

        valid_speeds = [100, 1000]
        if self._phoenix_account_type.is_futures:
    ...
    async def _subscribe_bars(self, bar_type: BarType) -> None:
        ...
        if not bar_type.spec.is_time_aggregated():
            self._log.error(
                f"Cannot subscribe to {bar_type}: only time bars are aggregated by Phoenix.",
            )
            return

        resolution = self._enum_parser.parse_nautilus_bar_aggregation(bar_type.spec.aggregation)
        if self._phoenix_account_type.is_futures and resolution == "s":
            self._log.error(
                f"Cannot subscribe to {bar_type}. "
                "Second interval bars are not aggregated by Phoenix Futures.",
            )
        try:
            interval = PhoenixKlineInterval(f"{bar_type.spec.step}{resolution}")
        except ValueError:
            self._log.error(
                f"Bar interval {bar_type.spec.step}{resolution} not supported by Phoenix.",
            )
            return
    ...
    async def _unsubscribe_order_book_deltas(self, instrument_id: InstrumentId) -> None:
        pass  # TODO: Unsubscribe from Phoenix if no other subscriptions
    ...
    async def _unsubscribe_bars(self, bar_type: BarType) -> None:
        if not bar_type.spec.is_time_aggregated():
            self._log.error(
                f"Cannot unsubscribe from {bar_type}: only time bars are aggregated by Phoenix.",
            )
            return
    ...
    async def _request_quote_ticks(
        ...
        self._log.error(
            "Cannot request historical quote ticks: not published by Phoenix.",
        )
    ...
    async def _request_bars(  # (too complex)
        ...
        if bar_type.spec.price_type != PriceType.LAST:
            self._log.error(
                ...
                f"only historical bars for LAST price type available from Phoenix.",
            )
            return
        ...
        if bar_type.is_externally_aggregated() or bar_type.spec.is_time_aggregated():
            if not bar_type.spec.is_time_aggregated():
                self._log.error(
                    f"Cannot request {bar_type}: only time bars are aggregated by Phoenix.",
                )
                return

            resolution = self._enum_parser.parse_nautilus_bar_aggregation(bar_type.spec.aggregation)
            if not self._phoenix_account_type.is_spot_or_margin and resolution == "s":
                self._log.error(
                    f"Cannot request {bar_type}: "
                    "second interval bars are not aggregated by Phoenix Futures.",
                )
            try:
                ...
                self._log.error(
                    f"Cannot create Phoenix Kline interval. {bar_type.spec.step}{resolution} "
                    "not supported.",
                )
                return

            bars = await self._http_market.request_phoenix_bars(
                bar_type=bar_type,
                interval=interval,
                start_time=start_time_ms,
                end_time=end_time_ms,
    ...
    async def _aggregate_internal_from_minute_bars(
        ...
        self._log.info("Requesting 1-MINUTE Phoenix bars to infer INTERNAL bars...", LogColor.BLUE)

        phoenix_bars = await self._http_market.request_phoenix_bars(
            bar_type=BarType(
                bar_type.instrument_id,
                BarSpecification(1, BarAggregation.MINUTE, PriceType.LAST),
                AggregationSource.EXTERNAL,
            ),
            interval=PhoenixKlineInterval.MINUTE_1,
            start_time=start_time_ms,
            end_time=end_time_ms,
            ts_init=self._clock.timestamp_ns(),
            limit=limit,
        ...
        for phoenix_bar in phoenix_bars:
            if phoenix_bar.count == 0:
                continue
            self._aggregate_bar_to_trade_ticks(
                instrument=instrument,
                aggregator=aggregator,
                phoenix_bar=phoenix_bar,
                quantize_value=quantize_value,
            )

        self._log.info(
            f"Inferred {len(bars)} {bar_type} bars aggregated from {len(phoenix_bars)} 1-MINUTE Phoenix bars.",
            LogColor.BLUE,
        )
    ...
    def _aggregate_bar_to_trade_ticks(
        ...
        phoenix_bar: PhoenixBar,
        quantize_value: Decimal,
    ) -> None:
        volume = phoenix_bar.volume.as_decimal()
        size_part: Decimal = (volume / (4 * phoenix_bar.count)).quantize(
            quantize_value,
            rounding=decimal.ROUND_DOWN,
        )
        remainder: Decimal = volume - (size_part * 4 * phoenix_bar.count)

        size = Quantity(size_part, instrument.size_precision)

        for i in range(phoenix_bar.count):
            open = TradeTick(
                instrument_id=instrument.id,
                price=phoenix_bar.open,
                size=size,
                aggressor_side=AggressorSide.NO_AGGRESSOR,
                trade_id=TradeId("NULL"),  # N/A
                ts_event=phoenix_bar.ts_event,
                ts_init=phoenix_bar.ts_event,
            )

            high = TradeTick(
                instrument_id=instrument.id,
                price=phoenix_bar.high,
                size=size,
                aggressor_side=AggressorSide.NO_AGGRESSOR,
                trade_id=TradeId("NULL"),  # N/A
                ts_event=phoenix_bar.ts_event,
    ...
    def _get_cached_instrument_id(self, symbol: str) -> InstrumentId:
        # Parse instrument ID
        phoenix_symbol = PhoenixSymbol(symbol)
        assert phoenix_symbol
        nautilus_symbol: str = phoenix_symbol.parse_as_nautilus(
            self._phoenix_account_type,
        )
        instrument_id: InstrumentId | None = self._instrument_ids.get(nautilus_symbol)
        if not instrument_id:
            instrument_id = InstrumentId(Symbol(nautilus_symbol), PHOENIX_VENUE)
    ...
    def _handle_ticker(self, raw: bytes) -> None:
        ...
        instrument_id: InstrumentId = self._get_cached_instrument_id(msg.data.s)
        ticker: PhoenixTicker = msg.data.parse_to_phoenix_ticker(
            instrument_id=instrument_id,
            ts_init=self._clock.timestamp_ns(),
        )
        data_type = DataType(
    ...
    def _handle_kline(self, raw: bytes) -> None:
        ...
        bar: PhoenixBar = msg.data.k.parse_to_phoenix_bar(
            instrument_id=instrument_id,
