...
from nautilus_trader.adapters.phoenix.common.enums import PhoenixExchangeFilterType
from nautilus_trader.adapters.phoenix.common.enums import PhoenixKlineInterval
from nautilus_trader.adapters.phoenix.common.enums import PhoenixRateLimitInterval
from nautilus_trader.adapters.phoenix.common.enums import PhoenixRateLimitType
...
class PhoenixTime(msgspec.Struct, frozen=True):
    """
...
class PhoenixDepth(msgspec.Struct, frozen=True):
    """
    Schema of a phoenix orderbook depth.
...
class PhoenixTrade(msgspec.Struct, frozen=True):
    """
    ...
    def parse_to_trade_tick(
        ...
        """
        Parse Phoenix trade to internal TradeTick.
        """
        return TradeTick(
...
class PhoenixAggTrade(msgspec.Struct, frozen=True):
    """
...
class PhoenixKline(msgspec.Struct, array_like=True):
    """
    Array-like schema of single Phoenix kline.
    """
    ...
    def parse_to_phoenix_bar(
        self,
        bar_type: BarType,
        ts_init: int,
    ) -> PhoenixBar:
        """
        Parse kline to PhoenixBar.
        """
        return PhoenixBar(
            bar_type=bar_type,
...
class PhoenixTicker24hr(msgspec.Struct, frozen=True):
    """
    Schema of single Phoenix 24hr ticker (FULL/MINI).
    """
...
class PhoenixTickerPrice(msgspec.Struct, frozen=True):
    """
    Schema of single Phoenix Price Ticker.
    """
...
class PhoenixTickerBook(msgspec.Struct, frozen=True):
    """
    Schema of a single Phoenix Order Book Ticker.
    """
...
class PhoenixDataMsgWrapper(msgspec.Struct):
    """
    Provides a wrapper for data WebSocket messages from `Phoenix`.
    """
...
class PhoenixOrderBookDelta(msgspec.Struct, array_like=True):
    """
...
class PhoenixOrderBookData(msgspec.Struct, frozen=True):
    """
    WebSocket message 'inner struct' for `Phoenix` Partial & Diff.
    ...
    b: list[PhoenixOrderBookDelta]  # Bids to be updated
    a: list[PhoenixOrderBookDelta]  # Asks to be updated
...
class PhoenixOrderBookMsg(msgspec.Struct, frozen=True):
    """
    WebSocket message from `Phoenix` Partial & Diff.
    ...
    stream: str
    data: PhoenixOrderBookData


class PhoenixQuoteData(msgspec.Struct, frozen=True):
    """
    WebSocket message from `Phoenix` Individual Symbol Book Ticker Streams.
    """
...
class PhoenixQuoteMsg(msgspec.Struct, frozen=True):
    """
    WebSocket message from `Phoenix` Individual Symbol Book Ticker Streams.
    """

    stream: str
    data: PhoenixQuoteData


class PhoenixAggregatedTradeData(msgspec.Struct, frozen=True):
    """
    WebSocket message from `Phoenix` Aggregate Trade Streams.
    """
...
class PhoenixAggregatedTradeMsg(msgspec.Struct, frozen=True):
    """
    WebSocket message.
    """

    stream: str
    data: PhoenixAggregatedTradeData


class PhoenixTickerData(msgspec.Struct, kw_only=True, frozen=True):
    """
    WebSocket message from `Phoenix` 24hr Ticker.
    ...
    def parse_to_phoenix_ticker(
        self,
        instrument_id: InstrumentId,
        ts_init: int,
    ) -> PhoenixTicker:
        return PhoenixTicker(
            instrument_id=instrument_id,
...
class PhoenixTickerMsg(msgspec.Struct, frozen=True):
    """
    WebSocket message.
    """

    stream: str
    data: PhoenixTickerData


class PhoenixCandlestick(msgspec.Struct, frozen=True):
    """
    WebSocket message 'inner struct' for `Phoenix` Kline/Candlestick Streams.
    ...
    s: str  # Symbol
    i: PhoenixKlineInterval  # Interval
    f: int  # First trade ID
    L: int  # Last trade ID
    o: str  # Open price
    c: str  # Close price
    ...
    def parse_to_phoenix_bar(
        ...
        bar_type = BarType(
            instrument_id=instrument_id,
            bar_spec=enum_parser.parse_phoenix_kline_interval_to_bar_spec(self.i),
...
class PhoenixCandlestickData(msgspec.Struct, frozen=True):
    """
    ...
    E: int
    s: str
    k: PhoenixCandlestick


class PhoenixCandlestickMsg(msgspec.Struct, frozen=True):
    """
    WebSocket message for `Phoenix` Kline/Candlestick Streams.
    """

    stream: str
    data: PhoenixCandlestickData
