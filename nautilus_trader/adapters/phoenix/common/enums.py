# -------------------------------------------------------------------------------------------------
#  Copyright (C) 2024 808putname All rights reserved.
#
...
@unique
class PhoenixRateLimitType(Enum):
    """
    Represents a `Phoenix` rate limit type.
    """
...
@unique
class PhoenixRateLimitInterval(Enum):
    """
    Represents a `Phoenix` rate limit interval.
    """
...
@unique
class PhoenixKlineInterval(Enum):
    """
    Represents a `Phoenix` kline chart interval.
    """
...
@unique
class PhoenixExchangeFilterType(Enum):
    """
    Represents a `Phoenix` exchange filter type.
    """
...
@unique
class PhoenixSymbolFilterType(Enum):
    """
    Represents a `Phoenix` symbol filter type.
    """
...
class PhoenixAccountType(Enum):
    ...
    def is_margin(self):
        return self in (
            PhoenixAccountType.MARGIN,
            PhoenixAccountType.ISOLATED_MARGIN,
        )

    @property
    def is_spot_or_margin(self):
        return self in (
            PhoenixAccountType.SPOT,
            PhoenixAccountType.MARGIN,
            PhoenixAccountType.ISOLATED_MARGIN,
        )
    ...
    def is_futures(self) -> bool:
        return self in (
            PhoenixAccountType.USDT_FUTURE,
            PhoenixAccountType.COIN_FUTURE,
        )

@unique
class PhoenixOrderSide(Enum):
    """
    Represents a `Phoenix` order side.
    """
...
@unique
class PhoenixExecutionType(Enum):
    """
    Represents a `Phoenix` execution type.
    """
...
@unique
class PhoenixOrderStatus(Enum):
    """
    Represents a `Phoenix` order status.
    """
...
@unique
class PhoenixTimeInForce(Enum):
    """
    Represents a `Phoenix` order time in force.
    """
...
@unique
class PhoenixOrderType(Enum):
    """
    Represents a `Phoenix` order type.
    """
...
@unique
class PhoenixSecurityType(Enum):
    """
    Represents a `Phoenix` endpoint security type.
    """
...
@unique
class PhoenixNewOrderRespType(Enum):
    """
    Represents a `Phoenix` newOrderRespType.
    """
...
@unique
class PhoenixErrorCode(Enum):
    """
    Represents a `Phoenix` error code (covers futures).
    """
...
class PhoenixEnumParser:
    """
    Provides common parsing methods for enums used by the 'Phoenix' exchange.
    ...
    def __init__(self) -> None:
        ...
        self.ext_to_int_status = {
            PhoenixOrderStatus.NEW: OrderStatus.ACCEPTED,
            PhoenixOrderStatus.CANCELED: OrderStatus.CANCELED,
            PhoenixOrderStatus.PARTIALLY_FILLED: OrderStatus.PARTIALLY_FILLED,
            PhoenixOrderStatus.FILLED: OrderStatus.FILLED,
            PhoenixOrderStatus.NEW_ADL: OrderStatus.FILLED,
            PhoenixOrderStatus.NEW_INSURANCE: OrderStatus.FILLED,
            PhoenixOrderStatus.EXPIRED: OrderStatus.EXPIRED,
            PhoenixOrderStatus.EXPIRED_IN_MATCH: OrderStatus.CANCELED,  # Canceled due self-trade prevention (STP)
        }

        self.ext_to_int_order_side = {
            PhoenixOrderSide.BUY: OrderSide.BUY,
            PhoenixOrderSide.SELL: OrderSide.SELL,
        }
        ...
        self.ext_to_int_time_in_force = {
            PhoenixTimeInForce.FOK: TimeInForce.FOK,
            PhoenixTimeInForce.GTC: TimeInForce.GTC,
            PhoenixTimeInForce.GTX: TimeInForce.GTC,  # Convert GTX to GTC
            PhoenixTimeInForce.GTE_GTC: TimeInForce.GTC,  # Undocumented
            PhoenixTimeInForce.IOC: TimeInForce.IOC,
            PhoenixTimeInForce.GTD: TimeInForce.GTD,
        }
        self.int_to_ext_time_in_force = {
            TimeInForce.GTC: PhoenixTimeInForce.GTC,
            TimeInForce.GTD: PhoenixTimeInForce.GTD,
            TimeInForce.FOK: PhoenixTimeInForce.FOK,
            TimeInForce.IOC: PhoenixTimeInForce.IOC,
        }

    def parse_phoenix_order_side(self, order_side: PhoenixOrderSide) -> OrderSide:
        try:
            return self.ext_to_int_order_side[order_side]
        except KeyError:
            raise RuntimeError(  # pragma: no cover (design-time error)
                f"unrecognized Phoenix order side, was {order_side}",  # pragma: no cover
            )

    def parse_internal_order_side(self, order_side: OrderSide) -> PhoenixOrderSide:
        try:
    ...
    def parse_phoenix_time_in_force(self, time_in_force: PhoenixTimeInForce) -> TimeInForce:
        try:
            return self.ext_to_int_time_in_force[time_in_force]
        except KeyError:
            raise RuntimeError(  # pragma: no cover (design-time error)
                f"unrecognized Phoenix time in force, was {time_in_force}",  # pragma: no cover
            )

    def parse_internal_time_in_force(self, time_in_force: TimeInForce) -> PhoenixTimeInForce:
        try:
    ...
    def parse_phoenix_order_status(self, order_status: PhoenixOrderStatus) -> OrderStatus:
        try:
            return self.ext_to_int_status[order_status]
        except KeyError:
            raise RuntimeError(  # pragma: no cover (design-time error)
                f"unrecognized phoenix order status, was {order_status}",  # pragma: no cover
            )

    def parse_phoenix_order_type(self, order_type: PhoenixOrderType) -> OrderType:
        # Implement in child class
        raise NotImplementedError

    def parse_internal_order_type(self, order: Order) -> PhoenixOrderType:
        # Implement in child class
        raise NotImplementedError

    def parse_phoenix_bar_agg(self, bar_agg: str) -> BarAggregation:
        try:
            return self.ext_to_int_bar_agg[bar_agg]
        except KeyError:
            raise RuntimeError(  # pragma: no cover (design-time error)
                f"unrecognized phoenix kline resolution, was {bar_agg}",
            )
    ...
    def parse_phoenix_kline_interval_to_bar_spec(
        self,
        kline_interval: PhoenixKlineInterval,
    ) -> BarSpecification:
        step = kline_interval.value[:-1]
        phoenix_bar_agg = kline_interval.value[-1]
        return BarSpecification(
            step=int(step),
            aggregation=self.parse_phoenix_bar_agg(phoenix_bar_agg),
            price_type=PriceType.LAST,
        )

    def parse_phoenix_trigger_type(self, trigger_type: str) -> TriggerType:
        # Replace method in child class, if compatible
        raise NotImplementedError(  # pragma: no cover (design-time error)
            "Cannot parse phoenix trigger type (not implemented).",  # pragma: no cover
        )
